...
use voxel_convert::{
    convert_voxel_object_and_install, decode_voxel_object_conversion_request,
    MAX_CONVERSION_SOURCE_BYTES, MAX_VOXEL_OBJECT_CONVERSION_REQUEST_BYTES,
};
...
fn run() -> Result<(), String> {
    ...
    let request_bytes = read_bounded(
        &arguments.request,
        MAX_VOXEL_OBJECT_CONVERSION_REQUEST_BYTES as u64,
        "request",
    )?;
