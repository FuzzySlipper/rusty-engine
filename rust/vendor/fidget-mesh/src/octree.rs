...
impl Octree {
    /// Number of escaped finest-cell QEF solutions recovered to their Hermite mass point.
    /// Counts construction events, including cells later considered for merging.
    pub fn bounded_leaf_vertices(&self) -> u32 { self.bounded_leaf_vertices }
    ...
    fn build_inner_mt<F: Function + RenderHints + Clone>(
        ...
        for o in &out {
            root.bounded_leaf_vertices += o.octree.bounded_leaf_vertices;
    ...
    fn leaf(
        ...
        for vs in CELL_TO_VERT_TO_EDGES[mask.index()].iter() {
            ...
            if let Some(pos) = force_point {
                ...
                let (mut pos, mut err) = qef.solve();
                // Rusty Engine patch: leaf QEFs are unconstrained upstream.
                // Recover escaped solutions to the average of their own edge
                // crossings, which stays in this convex cell. Unlike clipping
                // an escaped point to a face, this does not collapse distinct
                // surface vertices onto a shared cell boundary.
                if !cell.bounds.contains(pos) {
                    let mass = qef.mass_point();
                    pos = CellVertex { pos: mass.xyz() / mass.w };
                    err = QEF_ERR_INVALID;
                    self.octree.bounded_leaf_vertices += 1;
                }
                verts.push(pos);
