...
logic, register live callbacks, or acquire Product Kernel meaning. `rusty
package` seals the selected wrapper policy and exact runtime closure, then
delegates selected Tauri realization to `desktop.rs`; headed proof remains a
separate owner.
...
- [`rusty-cli/src/desktop.rs`](../../rust/crates/rusty-cli/src/desktop.rs)
...
| `src/desktop.rs` | Selected Tauri wrapper staging/publication, native bridge/bootstrap, exact release-tree receipt, relocation, and install lifecycle |
...
- no overwrite of conflicting product source or repair during `check` and
  `doctor`; and
- no duplicate native renderer-preload loader: desktop staging preserves the
  generated browser `main.js` as `frontend/product-main.js`, replaces only the
  public startup entry, and includes both modules in the exact frontend hash
  and relocation receipt; and
