...
| Adapter response JSONL | 64 MiB | Liveness and calibrated control | The host retains one complete response and the browser parses one JSON document. The prior 32 MiB protocol-v1 default rejected the checked 54.5 MB `rusty-engine-voxels` high-fidelity project. The 64 MiB ceiling admits that corpus and remains finite. Crossing it rejects the exchange immediately with the observed byte count; the serial queue drains the old line without retention before writing the next request, preserving attribution without killing a conforming adapter. |
...
| Render-resource and host-file paths | 4096 UTF-8 bytes | Liveness | Rejects malformed path input before normalization, symlink checks, and traversal. It is well above ordinary host path sizes. |
...
| Host-file extension filters | 16 filters of at most 17 ASCII characters | Liveness | Prevents an unbounded request-controlled filter set while covering ordinary editor file selectors. ASCII-only validation makes the character, UTF-16-code-unit, and UTF-8-byte counts identical. Exact-limit and one-over host-service tests pin both ceilings. |
...
| Host-user-settings project root | 4096 UTF-16 code units after trimming | Liveness | Bounds JavaScript normalization and hashing before settings I/O. This selector ceiling is distinct from the UTF-8 byte bounds on traversed resource/host-file paths; an exact-limit multibyte regression pins that distinction. |
