...
| `svc-mesh` | `engine-rs/crates/services/svc-mesh` | Internalized; test fixture path adapted | Deterministic visible-surface meshing directly over the same `VoxelWorld` used by collision/navigation, with successor-owned same-material coplanar greedy merging. Its closure is `core-space`, `core-voxel`, `svc-volume`, and `svc-spatial`; output is a derived presentation payload. |
