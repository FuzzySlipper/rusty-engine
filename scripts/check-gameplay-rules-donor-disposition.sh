...
DISPOSITION="$ENGINE_ROOT/migration/gameplay-rules-donor/disposition.tsv"
META="$ENGINE_ROOT/migration/gameplay-rules-donor/source.meta"

for required in "$DISPOSITION" "$META"; do
  if [[ ! -f "$required" ]]; then
    echo "missing gameplay rules donor accounting: $required" >&2
    exit 1
  fi
done
...
if [[ "$(head -n 1 "$DISPOSITION")" != "$expected_header" ]]; then
  echo "invalid gameplay rules donor disposition header" >&2
  exit 1
fi
...
awk -F '\t' '
  NR == 1 { next }
  NF != 5 {
    printf "invalid donor disposition field count at line %d\n", NR > "/dev/stderr"
    failed = 1
  }
  $1 == "" || $3 == "" || $4 == "" || $5 == "" {
  ...
  $2 != "adapted" && $2 != "evidence" && $2 != "excluded" {
...
for kind in adapted evidence excluded; do
  count="$(awk -F '\t' -v kind="$kind" \
    'NR > 1 && $2 == kind { count += 1 } END { print count + 0 }' \
    "$DISPOSITION")"
  if (( count == 0 )); then
...
if rg -n $'\t(pending|copied|compatible)\t' "$DISPOSITION"; then
  echo "gameplay rules donor accounting contains a non-final or compatibility disposition" >&2
  exit 1
fi

echo "gameplay rules donor disposition passed: $actual_count items, no pending rows"
