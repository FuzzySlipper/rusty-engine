...
| M1 | Ready next | Navigation projection, authored navigation intent, autonomous enemy route following, replanning, blocked/unreachable outcomes | M0 | `svc-pathfinding` candidate; `rule-lifecycle/fps_movement.rs` as evidence only | A visible enemy navigates around authored collision, resumes identically after reopen, and produces typed arrival/blocked/unreachable facts |
| M2A | Queued | Resolved input, player controller, look/move intent, authoritative pose, derived camera | M0 | Mine `protocol-input`, `rule-input`, and `protocol-view`; do not import lifecycle routing | Keyboard/mouse input moves one player through collision in the browser; bindings are content; camera owns no gameplay truth |
